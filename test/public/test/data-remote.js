...
asyncTest('submitting form with data-remote attribute', 2, function() {
  $('form[data-remote]')
    .append($('<input type="submit" name="commit" value="Button Text" />'))
    .bind('ajax:success', function(e, data, status, xhr) {
      App.assert_callback_invoked('ajax:success');
      equal(data.params.commit, 'Button Text', 'ajax arguments should have key commit with right value');
    })
    .bind('ajax:complete', function() { start() })
